...
public class Menu {
    ...
    public void menuResumo(User usuarioSelecionado) {
        ...
        switch (Integer.parseInt(in)) {
            case 1:
                printaResumo(usuarioSelecionado);
                return;

            case 2:
                criarConta(usuarioSelecionado);
                return;

            case 3:
                removerConta(usuarioSelecionado);
                return;

            case 4:
                registrarMovimento(usuarioSelecionado);
                return;
        ...
        while (true) {
            if (in.matches("[0-9]+")) {//Checa se a string tem apenas numeros.
                if (Integer.parseInt(in) <= numeroContas.size()) {//Checa se o numero é valido.
                    ...
                    System.out.println("Não existe uma conta com este numero. Digite novamente.");
                    in = sc.nextLine();
                }
    ...
    public void registrarMovimento(User usuarioSelecionado) {
        ContaDAO contaDAO = new ContaDAO();
        double valorMovimentado;
        System.out.println("Para registrar saldo positivo digite 1, para saldo negativo digite 2, para sair digite 0:");
        String opcao = sc.nextLine();
        switch (opcao) {
            case "1":
                System.out.println("Digite a quantia a adicionar: ");
                String valorAdicionado = sc.nextLine();
                while (!valorAdicionado.matches("[0-9\\.]+")) {
                    System.out.println("O valor deve conter somente numeros e um ponto.");
                    System.out.println("Digite o valor a ser adicionado:");
                    valorAdicionado = sc.nextLine();
                }
                valorMovimentado = Double.parseDouble(valorAdicionado);

                contaDAO.adicionaSaldo(selecionaConta(usuarioSelecionado),valorMovimentado);
                menuResumo(usuarioSelecionado);
                return;

            case "2":
                System.out.println("Digite a quantia a subtrair: ");
                String valorSubtraido = sc.nextLine();
                while (!valorSubtraido.matches("[0-9\\.]+")) {
                    System.out.println("O valor deve conter somente numeros e um ponto.");
                    System.out.println("Digite o valor a ser subtraido:");
                    valorSubtraido = sc.nextLine();
                }
                valorMovimentado = Double.parseDouble(valorSubtraido);

                contaDAO.diminuiSaldo(selecionaConta(usuarioSelecionado),valorMovimentado);
                menuResumo(usuarioSelecionado);
                return;

            case "0": menuResumo(usuarioSelecionado); break;
    ...
    public Conta selecionaConta(User usuarioSelecionado) {
        DatabaseDAO databaseDAO = new DatabaseDAO();
        Conta retorno = null;
        int i;
        ArrayList<String> numeroContas = databaseDAO.retornaNumeroContaUsuarioEspecifico(usuarioSelecionado.getCPF());
        ...
        while (true) {
            if (in.matches("[0-9]+")) {//Checa se a string tem apenas numeros.
                if (Integer.parseInt(in) <= numeroContas.size()) {//Checa se o numero é valido.
                    retorno = databaseDAO.retornaObjetoConta(numeroContas.get(Integer.parseInt(in) - 1));
                    break;
                } else {
                    System.out.println("Não existe um usuario com este numero. Digite novamente.");
                    in = sc.nextLine();
